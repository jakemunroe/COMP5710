...
def trackTaint(val2track, df_list_param):
    var_df, call_df, func_def_df, func_var_df = df_list_param
    ...
    func_trace = {}
    for _, row in func_def_df.iterrows():
        func_trace[row['TYPE'][-1]] = row['ARG_NAME']

    trace = [str(val2track)]
    data = val2track
    while taint_trace.get(data):
        trace.append(str(taint_trace[data]))
        data = taint_trace[data]

    trace.append("res")
    trace = trace[-3:]  # Adjust to include v1

    # Add v1
    if 'v1' in taint_trace:
        trace.insert(2, 'v1')
